...
class SymmetricTree:
    ...
    def create(self, shape, namespace = None, filename = None):
        ...
        for sx in range(self._shape[-1]):
            ...
            if ondisk:
                ...
                path_array = np.ndarray((length,),dtype=np_type)
    ...
    def _alloc_path(self, path):
        ...
        #get level info
        shape = self._shape[path[1] * SymmetricTree.SHAPE_SIZE + SymmetricTree.LOC_SHAPE] #shape
        total = self._shape[path[1] * SymmetricTree.SHAPE_SIZE + SymmetricTree.LOC_TOTAL] #total
        used = self._shape[path[1] * SymmetricTree.SHAPE_SIZE + SymmetricTree.LOC_USED] #used

        #ensure the new path does not overflow maximum array size
        assert (used + 1 ) <= 4294967295 , "Cannot allocate space for path {} - new size value {}+1 will overflow maxint".format(path,used)

        #ensure the new path does not overflow our allocated size
        assert ( used + 1 ) * shape <= total, "Cannot allocate space for path {} - maximum individual level size exceeded".format(path)
    ...
    def set(self, path, value = None, mathop = None):
        ...
        for px in range(len(path)):
            ...
            #if we are on the last path, just set the value, we are done
            if px == len(path)-1:
    ...
    def get(self, path, default=None, items=1, set_default = False, include_type = False):
        ...
        for px in range(len(path)):
            ...
            if px > 0:
                if index_shape != p[1]:
                    assert False, "Path: {}... Mismatched Shape at Leaf. Expected {} Found {}".format(
                        path[:px],
                        p[1],
                        index_shape
                    )
            ...
            #if we are on the last path, just get the value, we are done
            if px == len(path)-1:
                ...
                if real_index > self._shape[path[px][1] * SymmetricTree.SHAPE_SIZE + SymmetricTree.LOC_TOTAL]:
                    assert False, "Path {}: Index Overflow".format(path[:px])
